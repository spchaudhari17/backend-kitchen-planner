...
exports.getAll = async (req, res, next) => {
    try {
        const query = req.roles.includes(ROLES_LIST.Staff) ? {} : { $or: [{ roles: ROLES_LIST.User }, { _id: req.user._id }], roles: { $ne: ROLES_LIST.Staff }}
...
exports.update = async (req, res, next) => {
    try {
        ...
        if(verifyRole.roles.includes(ROLES_LIST.Staff)) throw new CustomError('Not authorized to edit Staff user', 401)
...
exports.delete = async (req, res, next) => {
    try {
        ...
        const verifyRole = await User.findById(id).lean().exec()
        if(verifyRole.roles.includes(ROLES_LIST.Staff)) throw new CustomError('Not authorized to delete Staff user', 401)
...
exports.Guest = async (req, res, next) => {
    try {
        const mockData = [
            { id: 1, title: "Plan A", description: "A basic kitchen plan.", price: "$200" },
            { id: 2, title: "Plan B", description: "An advanced kitchen plan.", price: "$400" },
        ];

        res.status(200).json({
            message: "Welcome, Guest! Here is the publicly available content.",
            data: mockData,
        });
    } catch (error) {
        next(error);
