...
const setupSocket = (io) => {
  ...
  const updateUserLists = async () => {
    ...
    const staffSession = userSessions.get('Staff')
    ...
    if (staffSession) {
      ...
      io.to(staffSession.socketId).emit('adminUpdateUserList', users)
    }
  }
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('online', async (userId) => {
      ...
      if (user.roles.includes(ROLES_LIST.Staff)) {
        console.log('Staff connected:', socket.id)
        userSessions.set('Staff', { id: user._id, socketId: socket.id })
        socket.join(socket.id)
      }
